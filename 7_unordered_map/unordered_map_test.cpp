...
void SimpleTest() {
  ...
  assert(m.at("aaaaa") == 4);
  // std::cerr << "decrementing each value passed, value at aaaaa == 4" <<
  // std::endl;
...
void TestIterators() {
  ...
  assert(m.size() == 8);
  // Actions below must be quick (~ 1000 * 8 operations) despite reserving space
  // for 1M elements
  ...
  ++it;
  s = it->second;
  // I asked to reserve space for 1M elements so actions below adding 100'000
  // elements mustn't cause reallocation
...
void TestNoRedundantCopies() {
  ...
  UnorderedMap<NeitherDefaultNorCopyConstructible,
               NeitherDefaultNorCopyConstructible>
      m;
  ...
  // std::cerr << "m.emplace(VerySpecialType(0), VerySpecialType(0)) done" <<
  // std::endl;
  m.reserve(1'000'000);
  // std::cerr << "m.reserve(1000000) done" << std::endl;
  std::pair<NeitherDefaultNorCopyConstructible,
            NeitherDefaultNorCopyConstructible>
      p{VerySpecialType(1), VerySpecialType(1)};
  ...
  // std::cerr << "m.at(VerySpecialType(1)) = VerySpecialType(0) done" <<
  // std::endl;
...
bool operator==(const OneMoreStrangeStruct&,
                const OneMoreStrangeStruct&) = delete;
...
int main() {
  ...
  std::cerr << "TestConstIteratorDoesntAllowModification (3 of 6) passed"
            << std::endl;
