...
TestGroup create_constructor_tests() {
  return {"construction and assignment",
          ...
          make_test<PrettyTest>(
              ...
              [](auto& test) {
                using custom_alloc_type = NotPropagatedCountingAllocator<
                    std::pair<const int, NeitherDefaultNorCopyConstructible>>;
                using special_map_type =
                    UnorderedMap<int, NeitherDefaultNorCopyConstructible,
  ...
  return {
      ...
      make_test<PrettyTest>("emplace",
                            [](auto& test) {
                              ...
                              auto [new_place, new_did_insert] =
                                  map.emplace(2, 2_ntr);
                              // update place as it could be invalidated by
                              // rehash
                              place = map.find(1);
      ...
      make_test<PrettyTest>("move insert range", [](auto& test) {
        ...
        std::transform(indices.begin(), indices.end(),
                       std::back_inserter(storage), [](int idx) {
        ...
        map.insert(std::move_iterator(storage.begin()),
                   std::move_iterator(storage.end()));
        test.equals(storage.size(), small_size);
        test.check(rng::all_of(
            storage, [&](auto& p) { return test.equals(p.second, ""); }));
      })};
}
...
TestGroup create_access_tests() {
  return {"access",
          make_test<PrettyTest>(
              ...
              [](auto& test) {
                /*auto range = iota(0, int(small_size))
                    | transform([](int item) -> std::pair<const int,
                   std::string> { return {item, std::to_string(item)};
                    })
                    | common;*/
                std::vector<int> indices(small_size);
                std::iota(indices.begin(), indices.end(), 0);
                ...
                std::transform(indices.begin(), indices.end(),
                               std::back_inserter(range), [](int idx) {
                                 return std::pair<const int, std::string>{
                                     idx, std::to_string(idx)};
                               });
...
TestGroup create_misc_tests() {
  return {"misc", make_test<PrettyTest>("load factor", [](auto& test) {
            ...
            auto max_val = std::max_element(map.begin(), map.end(),
            ...
            for (auto i = max_val + 1; i < max_val + 1 + int(medium_size);
                 ++i) {
